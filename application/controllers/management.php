...
//        $groupid = $this->input->post('group');
        ...
        $is_valid = $this->input->post('is_valid');
        $limitime = $this->input->post('limitime');
        $uid = $this->input->post('uid');
        $tag = $this->input->post('tag');

        $group_text = $this->input->post('group_text');

        $group_re = $this->input->post('group_value');
        $group_value = intval($group_re, 10);

        if($tag == 'true'){//修改密码
            $res = $this->mmanagement->update_user_changepassword($db, $username, md5($password), $is_valid, $uid);
        }else {//不修改密码
            $res = $this->mmanagement->update_user_holdpassword($db, $username, $is_valid, $uid);
        }

        $this->mmanagement->set_limitime($limitime, $username); // 设置用户使用时限
//        var_dump($group_value);
        $this->mmanagement->set_groupid($group_value, $group_text, $username); // 设置用户权限
